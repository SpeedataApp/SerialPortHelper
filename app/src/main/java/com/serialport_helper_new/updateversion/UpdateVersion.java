package com.serialport_helper_new.updateversion;
...
public class UpdateVersion {
    ...
    public class CheckVersionTask implements Runnable {
        ...
        public void run() {
            try {
                // 从资源文件获取服务器 地址
                // String path = getResources().getString(R.string.);
                // http://218.247.237.138:8090/kt40helper/version.xml
                // http://218.247.237.138/kt40helper/version.xml
                String path = "http://218.247.237.138:9002/speedataApps/serialHelperVersion.xml";
                // String path = "http://10.123.10.10:997/update/version.xml";
                // 包装成url的对象
                ...
                // 待处理
                e.printStackTrace();
                System.out.println("=======GET_UNDATAINFO_ERROR===" + e.getMessage());
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case UPDATA_NONEED:
                    ToastUtils.showShortToastSafe("版本号相同无需升级");
                    break;
                case UPDATA_CLIENT:
                    // 对话框通知用户升级程序
                    ToastUtils.showShortToastSafe("可以升级程序啦");
                    downLoadApk();
                    // showUpdataDialog();
                    break;
                case GET_UNDATAINFO_ERROR:
                    // 服务器超时
                    ToastUtils.showShortToastSafe("获取服务器更新信息失败");
                    // LoginMain();
                    break;
                case SDCARD_NOMOUNTED:
                    // sdcard不可用
                    ToastUtils.showShortToastSafe("SD卡不可用");
                    break;
                case DOWN_ERROR:
                    // 下载apk失败
                    ToastUtils.showShortToastSafe("下载新版本失败");
                    // LoginMain();
                    break;
                default:
                    break;
            ...
            new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        pd.dismiss();
                        System.out.println("DOWN_ERROR====" + e.getMessage());
                        Log.d(TAG, "DOWN_ERROR " + e.getMessage());
                        Message msg = new Message();
                        msg.what = DOWN_ERROR;
                        handler.sendMessage(msg);
                        e.printStackTrace();
